...
func (c *ClientCmd) save(ctx context.Context, globals *Globals) error {
	...
	createResp, err := cl.CreateCacheEntryWithResponse(ctx, api.CreateCacheEntryJSONRequestBody{
	...
	updateResp, err := cl.UpdateCacheEntryWithResponse(ctx, api.CacheEntryUpdateRequest{
		Id:  id,
		Key: fileName,
